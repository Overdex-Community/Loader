...
local function sendWebhook()
    ...
    if typeof(eq) == "table" then
        ...
        if typeof(w) == "table" then
            ...
            if w["Type"] or w.Type then
                local t = tostring(w["Type"] or w.Type)
                types[#types+1] = t
                seen[t] = true
            end

            for _, v in pairs(w) do
                if typeof(v) == "table" and (v["Type"] or v.Type) then
                    local t = tostring(v["Type"] or v.Type)
                    if not seen[t] then
                        seen[t] = true
                        types[#types+1] = t
                    end
    ...
    if typeof(inv) == "table" then
        ...
        if typeof(eqs) == "table" then
            for _, it in pairs(eqs) do
                if typeof(it) == "table" and (it["Type"] or it.Type) then
                    invEquips[#invEquips+1] = tostring(it["Type"] or it.Type)
                end
            end
        end
    end
